...
export function RecipesView({ userId, recetas, tiposDieta, recetasFavoritasIds }: RecipesViewProps) {
  ...
  const recetasFiltradas = recetas.filter((receta) => {
    ...
    const coincideEtiqueta =
      !filtroEtiqueta || receta.receta_etiquetas.some((e) => e.etiqueta.toLowerCase() === filtroEtiqueta.toLowerCase())
  ...
  const recetasRecomendadas = recetasFiltradas.filter((receta) => {
    ...
    return receta.receta_etiquetas.some((e) => tiposDieta.some((td) => td.toLowerCase() === e.etiqueta.toLowerCase()))
  })

  const etiquetasUnicas = Array.from(
    new Set(recetas.flatMap((r) => r.receta_etiquetas.map((e) => e.etiqueta.toLowerCase()))),
  ).sort()
